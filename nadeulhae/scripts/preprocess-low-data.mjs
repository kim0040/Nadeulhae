...
const MISCLASSIFIED_FIXES = [
  ...
  // 카페/베이커리 → cafe
  {
    keywords: ["커피", "카페", "베이커리", "제과", "도넛", "브런치", "디저트"],
    category: "cafe",
  },
  // 편의점 → shopping
  {
...
function tagPlaceType(category, name) {
  ...
  if (INDOOR_CATEGORIES.has(category)) {
    ...
    if (OUTDOOR_PLACE_KEYWORDS.some((k) => name.includes(k)))
      return "semi-outdoor";
    return "indoor";
  }
  // attraction은 장소명 기반
...
function makePlace(overrides) {
  ...
  for (const t of p.weatherTags)
    stats.weatherTags[t] = (stats.weatherTags[t] || 0) + 1;
  return p;
}
...
function processFestivals(places) {
  const { rows } = parseCSV(
    fs.readFileSync(path.join(DATA_DIR, "jeonju_festival_detail.csv"), "utf8"),
  );
...
function processSoccer(places) {
  const { rows } = parseCSV(
    fs.readFileSync(path.join(DATA_DIR, "jeonju_soccer.csv"), "utf8"),
  );
...
function processPlaceDB(places) {
  const { rows } = parseCSV(
    fs.readFileSync(path.join(DATA_DIR, "장소DB(최종).csv"), "utf8"),
  );
...
function processPayAfter(places) {
  const { rows } = parseCSV(
    fs.readFileSync(path.join(DATA_DIR, "jeonju_pay_after_최종.csv"), "utf8"),
  );
...
function processRestaurants(places) {
  const { headers, rows } = parseCSV(
    fs.readFileSync(
      path.join(DATA_DIR, "전주시_음식점_크롤링결과1.csv"),
      "utf8",
    ),
  );
  ...
  for (const r of rows) {
    ...
    const reviewCount = safeNum(
      safeStr(r[idx.kakaoReview])?.replace(/[^\d]/g, ""),
    );

    // 메뉴
    const menuNames = [
      safeStr(r[idx.menu1Name]),
      safeStr(r[idx.menu2Name]),
      safeStr(r[idx.menu3Name]),
    ].filter(Boolean);
    const menuPrices = [
      normalizePrice(safeStr(r[idx.menu1Price])),
      normalizePrice(safeStr(r[idx.menu2Price])),
      normalizePrice(safeStr(r[idx.menu3Price])),
    ];
    ...
    places.push(
      makePlace({
        ...
        reviewCount,
        address:
          safeStr(r[idx.kakaoAddress]) ||
          safeStr(r[idx.roadAddr]) ||
          safeStr(r[idx.jibunAddr]),
...
// =========================================================================
// 자체 지오코딩: 좌표 있는 데이터로 없는 데이터 보강 (API 불필요)
// 전략 1: 이름 정확 일치 → 2: 포함관계 + 주소 앞부분 일치
// =========================================================================
console.log('\n자체 지오코딩 시작...');
...
for (const p of allPlaces) {
  ...
  if (!coordLookup.has(key)) coordLookup.set(key, []);
  coordLookup.get(key).push({ lat: p.lat, lon: p.lon, address: (p.address || '').trim() });
}

const normAddr = (s) => (s || '').replace(/전북특별자치도|전라북도|전주시 완산구|전주시 덕진구/g, '').trim();

let geoCoded = 0;
for (const p of allPlaces) {
  if (p.lat && p.lon) continue;
  const name = p.name.trim();
  const addr = normAddr(p.address);

  const exact = coordLookup.get(name);
  if (exact) {
    const best = exact.find(c => normAddr(c.address) === addr) || exact[0];
    p.lat = best.lat; p.lon = best.lon;
    if (p.extra) { p.extra.geoCodedFrom = best.address; delete p.extra.needsGeocoding; }
    geoCoded++;
    continue;
  }

  for (const [lookupName, entries] of coordLookup) {
    if (name.includes(lookupName) || lookupName.includes(name)) {
      const addrPrefix = (addr || '').slice(0, 8);
      for (const c of entries) {
        const cPrefix = normAddr(c.address).slice(0, 8);
        if (addrPrefix && cPrefix && addrPrefix === cPrefix) {
          p.lat = c.lat; p.lon = c.lon;
          if (p.extra) { p.extra.geoCodedFrom = c.address; delete p.extra.needsGeocoding; }
          geoCoded++;
          break;
        }
      }
...
stats.withCoords = allPlaces.filter(p => p.lat && p.lon).length;
stats.withoutCoords = allPlaces.length - stats.withCoords;
console.log(`  자체 지오코딩: ${geoCoded}건 보강 → 좌표 O: ${stats.withCoords} / X: ${stats.withoutCoords}`);
...
const report = {
  ...
  todo: [
    "✅ 자체 지오코딩: 이름 기반 매칭으로 API 없이 좌표 보강 완료. 미매칭 약 7,500건은 자연명소·축제장소로, 수동 또는 카카오 API 보완 가능",
...
console.log(`\nDone!`);
console.log(
  `  Raw: ${allPlaces.length} → Deduped: ${deduped.length} (${dupCount} duplicates)`,
);
console.log(
  `  With coords: ${stats.withCoords} / Without: ${stats.withoutCoords}`,
);
console.log(
  `  Output: src/data/places.json (${(fs.statSync(path.join(OUT_DIR, "places.json")).size / 1024 / 1024).toFixed(1)} MB)`,
);
