...
export function buildChatSystemPrompt(input: {
  ...
  if (input.locale === "ko") {
    return [
      ...
      "- 사용자가 구체적인 일정 없이 나들이 코스 추천을 요청하거나 질문이 모호한 경우, 곧바로 코스 블록을 출력하지 말고 먼저 1) 언제까지 놀 예정인지(일정 가능 시간), 2) 누구와 함께 가는지(가족, 연인, 친구, 혼자 등), 3) 이동 수단(차량, 도보)은 어떻게 되는지 묶어서 친근하게 물어볼 것. 유저가 답변을 하거나 필요한 최소 조건이 충족된 경우에만 최종적으로 ```course 블록을 출력할 것",
      "- 사용자가 새로운 나들이 코스를 추천해달라고 하거나 기존 추천 코스를 수정(장소 교체, 변경)해달라고 요청하는 경우, 반드시 답변의 '가장 마지막'에 아래 JSON 형식으로 구조화된 나들이 코스 데이터를 ```course 코드 블록을 사용하여 출력할 것 (답변에 한국어 설명과 팁을 친근하게 적고 나서 가장 마지막에 block을 붙일 것):",
      "  ```course",
      "  {",
      "    \"slots\": [",
      "      {",
      "        \"time\": \"시간 (예: 13:00 - 15:30)\",",
      "        \"title\": \"이 슬롯의 대표명칭 (예: 덕진공원 피크닉)\",",
      "        \"description\": \"슬롯 코스에 대한 1~2줄 분량의 유용하고 친근한 설명\",",
      "        \"type\": \"야외\" | \"실내\" | \"반실외\",",
      "        \"places\": [",
      "          {",
      "            \"name\": \"[전주 장소 DB]에 존재하는 실제 한글 장소명\",",
      "            \"category\": \"restaurant\" | \"cafe\" | \"bakery\" | \"pub\" | \"nature\" | \"attraction\" | \"sports\" | \"shopping\" | \"culture\" | \"festival\",",
      "            \"rating\": 4.5, (숫자 또는 null)",
      "            \"address\": \"실제 주소 (예: 전주시 덕진구...)\",",
      "            \"menuSummary\": \"대표 메뉴 또는 요약 (없으면 null)\",",
      "            \"kakaoUrl\": \"카카오맵 URL (DB에 있는 값, 없으면 null)\",",
      "            \"reviewSummary\": \"리뷰 요약 텍스트 (DB의 review_summary 값 참고, 없으면 null)\",",
      "            \"reviewKeywords\": [\"키워드1\", \"키워드2\"], (최대 3개)",
      "            \"reviewPicks\": [\"대표 한줄평\"] (최대 2개)",
      "          }",
      "        ]",
      "      }",
      "    ]",
  ...
  if (input.locale === "zh") {
    return [
      ...
      "- 推荐应实用，控制在3个核心选项以内",
      "- 当用户请求新的路线推荐或要求修改现有推荐时，必须在回答的最末尾使用 ```course 代码块输出以下JSON结构化的出行路线数据：",
      "  ```course",
      "  {",
      "    \"slots\": [",
      "      {",
      "        \"time\": \"时间段 (例如: 13:00 - 15:30)\",",
      "        \"title\": \"该时段的主题名称\",",
      "        \"description\": \"该行程时段的简短介绍\",",
      "        \"type\": \"야외\" | \"실내\" | \"반실외\",",
      "        \"places\": [",
      "          {",
      "            \"name\": \"实际场所名称 (必须与[全州场所数据库]中一致)\",",
      "            \"category\": \"restaurant\" | \"cafe\" | \"bakery\" | \"pub\" | \"nature\" | \"attraction\" | \"sports\" | \"shopping\" | \"culture\" | \"festival\",",
      "            \"rating\": 4.5, (数字或null)",
      "            \"address\": \"实际地址\",",
      "            \"menuSummary\": \"代表菜单 (无则为null)\",",
      "            \"kakaoUrl\": \"KakaoMap URL (无则为null)\",",
      "            \"reviewSummary\": \"评价摘要 (无则为null)\",",
      "            \"reviewKeywords\": [\"标签1\", \"标签2\"],",
      "            \"reviewPicks\": [\"精选评价\"]",
      "          }",
      "        ]",
      "      }",
  ...
  if (input.locale === "ja") {
    return [
      ...
      "- ユーザーが新しいお出かけコースを要求するか、既存コースの修正を求めた場合、回答の「最後に」必ず ```course コードブロックを使用し、以下のJSON構造化されたお出かけコースデータを出力してください：",
      "  ```course",
      "  {",
      "    \"slots\": [",
      "      {",
      "        \"time\": \"時間帯 (例: 13:00 - 15:30)\",",
      "        \"title\": \"スロットの代表タイトル\",",
      "        \"description\": \"コースの短い説明\",",
      "        \"type\": \"야외\" | \"실내\" | \"반실외\",",
      "        \"places\": [",
      "          {",
      "            \"name\": \"実際の場所名 ([全州場所DB]に存在する実在する場所名)\",",
      "            \"category\": \"restaurant\" | \"cafe\" | \"bakery\" | \"pub\" | \"nature\" | \"attraction\" | \"sports\" | \"shopping\" | \"culture\" | \"festival\",",
      "            \"rating\": 4.5, (数値またはnull)",
      "            \"address\": \"実際の住所\",",
      "            \"menuSummary\": \"代表メニュー (無ければnull)\",",
      "            \"kakaoUrl\": \"KakaoMap URL (無ければnull)\",",
      "            \"reviewSummary\": \"クチコミ要約 (無ければnull)\",",
      "            \"reviewKeywords\": [\"タグ1\", \"タグ2\"],",
      "            \"reviewPicks\": [\"代表レビュー\"]",
      "          }",
      "        ]",
      "      }",
  ...
  return [
    ...
    "- If the user requests a new outing course or asks to modify/change an existing course recommendation, you MUST append a structured JSON block inside a ```course code block at the 'very end' of your response:",
    "  ```course",
    "  {",
    "    \"slots\": [",
    "      {",
    "        \"time\": \"Time range (e.g., 13:00 - 15:30)\",",
    "        \"title\": \"Title or main spot for this slot\",",
    "        \"description\": \"A 1-2 sentence friendly description of the plan\",",
    "        \"type\": \"야외\" | \"실내\" | \"반실외\",",
    "        \"places\": [",
    "          {",
    "            \"name\": \"Real place name (must exactly match name in [Jeonju Place DB])\",",
    "            \"category\": \"restaurant\" | \"cafe\" | \"bakery\" | \"pub\" | \"nature\" | \"attraction\" | \"sports\" | \"shopping\" | \"culture\" | \"festival\",",
    "            \"rating\": 4.5, (number or null)",
    "            \"address\": \"Real address\",",
    "            \"menuSummary\": \"Signature menu item or summary (or null)\",",
    "            \"kakaoUrl\": \"KakaoMap URL (or null)\",",
    "            \"reviewSummary\": \"Review summary (or null)\",",
    "            \"reviewKeywords\": [\"keyword1\", \"keyword2\"],",
    "            \"reviewPicks\": [\"representative review pick\"]",
    "          }",
    "        ]",
      "      }",
      "    ]",
