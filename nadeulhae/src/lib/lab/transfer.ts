...
function normalizeCardDraft(input: Record<string, unknown>): LabGeneratedCardInput | null {
  ...
  const tip = trimTo(normalizeCell(input.tip ?? input.note ?? input.memo ?? input.exampleMeaning ?? input.팁), 200)
...
export function parseLabImportPayload(input: {
  ...
  if (input.format === "json") {
    ...
    const cards = dedupeCards(
      cardsSource
        ...
        .filter((card): card is LabGeneratedCardInput => Boolean(card))
    )
    ...
    return {
      cards,
      totalParsedRows: cardsSource.length,
      sourceFormat: "json",
      deckTitle,
      deckTopic,
    }
  ...
  const headerTipIndex = pickIndex(headerMap, ["tip", "memo", "note", "examplemeaning", "팁"])
  ...
  const tipIndex = hasHeader ? headerTipIndex : 3
  ...
  for (let i = startIndex; i < rows.length; i += 1) {
    ...
    const draft = normalizeCardDraft({
      ...
      tip: typeof tipIndex === "number" ? row[tipIndex] ?? "" : "",
    })
...
export function buildLabTemplateCsv() {
  const rows = [
    ...
    ["breeze", "산들바람", "A cool breeze makes the trail pleasant.", "outdoor"],
    ["drizzle", "이슬비", "Light drizzle started near the river.", "rain"],
  ]
