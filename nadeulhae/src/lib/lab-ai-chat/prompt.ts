...
export function buildLabAiChatSystemPrompt(input: {
  ...
  if (input.locale === "ko") {
    ...
    "- 나들이나 피크닉 추천 전용 봇이 아니라 글쓰기, 코딩, 학습, 요약, 아이디어 정리까지 돕는 범용 대화형 AI로 답할 것",
      ...
      "- 사용자가 요청한 깊이에 맞춰 답하고, 불필요하게 장황하게 늘어놓지 말 것",
  ...
  return [
    ...
    "- Match the user's requested depth and avoid unnecessary verbosity",
...
export function buildLabAiChatSummaryPrompt(input: {
  ...
  return [
    ...
    "Rules:",
    "- Keep it within 8 short bullets or compact brief paragraphs",
