...
import { useFireSummary, useWeatherData } from "@/hooks/use-weather";
...
export default function JeonjuPage() {
  ...
  const texts = useMemo(() => {
    ...
    > = {
      ...
      zh: {
        ...
        heroDesc:
          "这是为 Nadeulhae 的故乡全州打造的数据驱动出行实验室。提供实时环境分析、天气简报、安全面板，以及每天早晨由NadeulAI整理的全州新闻，带来专属于全州的精细化本地体验。",
        fixedTitle: "全州固定简报 & 野餐日历",
        fixedDesc:
          "本区域的出行简报和10天日历始终仅以全州为基准进行计算和显示，无需位置追踪。",
      ...
      ja: {
        ...
        heroDesc:
          "Nadeulhae の故郷、全州のためのデータ駆動型お出かけラボです。リアルタイム環境分析、天気ブリーフィング、安全パネル、そして毎朝NadeulAIがまとめる全州ニュースまで、全州だけの精密なローカル体験を提供します。",
        fixedTitle: "全州固定ブリーフィング & ピクニックカレンダー",
        fixedDesc:
          "このエリアのお出かけブリーフィングと10日間カレンダーは、位置追跡なしで常に全州基準のみで計算・表示します。",
  ...
  return (
    <main className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      {/* Place tags marquee — locale-aware Jeonju landmarks */}{" "}
      ...
      {/* Daily briefing — AI-curated Jeonju news */}{" "}
      ...
      {/* Fixed Jeonju briefing + picnic calendar (no geolocation) */}{" "}
      <section className="mx-auto max-w-6xl px-4 pb-20 sm:px-6">
        <div className="mb-8 max-w-3xl">
          <h2 className="text-3xl sm:text-5xl font-black tracking-tight text-foreground">
            {texts.fixedTitle}
      ...
      {/* Safety panel — weather + fire data for Jeonju */}{" "}
      {weatherData && (
        <section className="mx-auto max-w-6xl px-4 pb-20 sm:px-6">
          <JeonjuSafetyPanel
