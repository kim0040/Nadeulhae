...
export function getAdviceText(dayForecast: ForecastDay, language: string): string {
  ...
  const la = (
    ...
    if (language === "zh") return pick(zhArr);
    if (language === "ja") return pick(jaArr);
    return language === "ko" ? pick(koArr) : pick(enArr);
  };
  ...
  if (isWetDay) {
    return la(
      ...
      [
        "傘は必須！雨の景色を楽しめるカフェがおすすめです。",
        "雨の日は全州韓屋村の屋内体験がぴったり。",
        "傘を持って全州川沿いのカフェでゆったり過ごしましょう。",
        "雨音を聞きながら全州の伝統茶はいかが？",
      ],
    );
    ...
    return la(
      ...
      [
        "今天很热，去凉爽的博物馆或购物中心如何？",
        "全州博物馆之旅正合适——在空调里感受文化！",
        "高温预警级别——户外活动最好安排在早晨或傍晚。",
        "推荐在室内体验全州传统工艺来避暑。",
      ],
      [
        "今日はかなり暑いです。涼しい博物館やショッピングモールはいかが？",
        "全州博物館ツアー에 ぴったりの日—エアコンの効いた室内で文化探訪！",
        "猛暑レベルのため、屋外活動は朝か夕方に延期した方が良さそうです。",
        "暑い日は室内で楽しめる全州の伝統工芸体験がおすすめ。",
      ],
    );
    ...
    return la(
      ...
      [
        "かなり暑いです。木陰の多い徳津公園の遊歩道がおすすめ。",
        "暑さを避けて全州の屋内スポットを巡るのはいかが？",
        "暖かい日は軽くお出かけした後に冷たいパッピンスを！",
      ],
    );
    ...
    return la(
      ...
      [
        "冷たい風が吹いています。温かいお茶と共に室内でゆったり過ごしましょう。",
        "寒い日は全州の伝統茶房で体を温めるのはいかが？",
        "寒波レベルなので、屋外は短めに、室内中心で計画を。",
      ],
    );
    ...
    return la(
      ...
      [
        "天气凉爽，带件薄外套去全州小巷散步吧。",
        "微凉但非常适合散步——推荐全州韩屋村小巷之旅！",
        "初春/初秋的氛围，适合轻松户外活动。",
      ],
      [
        "肌寒いですが散歩に良い日です。全州韓屋村의 路地巡り가 おすすめ！",
        "少し肌寒いけど散歩にぴったり。全州韓屋村の路地ツアーを！",
        "早春・初秋のような陽気。軽く屋外で過ごすのに良い日です。",
      ],
    );
    ...
    return la(
      ...
      [
        "野餐最佳日子！带上垫子立刻出发去公园吧。",
        "真正的出行日！去全州细碧湖骑行正合适。",
        "在晴空下沿着全州川散步，一整天都会很幸福。",
      ],
      [
        "ピクニックに最高の日！レジャーシートを持って今すぐ公園へ。",
        "今日は本当にお出かけ日和！全州セビョンホでサイクリングが最高。",
        "晴れた空の下、全州川沿いを歩けば幸せな一日に。",
      ],
    );
    ...
    return la(
      ...
      [
        "非常适合散步或轻度户外活动的好天气。",
        "沿着德津公园的荷花路悠闲散步吧——心情美好的一天！",
        "适合全州社区咖啡馆之旅的日子，步行享受吧。",
      ],
      [
        "散歩や軽い屋外活動にぴったりの天気です。",
        "徳津公園の蓮の道をゆったり歩いてみてください。気分の良い一日を！",
        "全州の街カフェ巡りに良い日です。歩いて楽しみましょう。",
      ],
    );
    ...
    return la(
      ...
      [
        "还算不错的出行天气，短暂外出转换心情很好。",
        "户外也可以，但准备一个室内备选会更好。",
        "推荐全州近郊的短途自驾路线。",
      ],
      [
        "まずまずのお出かけ日和。短い外出で気分転換に良いです。",
        "屋外も大丈夫ですが、室内の代替案も用意しておくとより良い一日に。",
        "全州近郊の短いドライブコースがおすすめです。",
      ],
    );
  } else {
    return la(
      ...
      [
        "适合轻松外出的天气，祝您愉快！",
        "以室内为主，短途散步也可以。",
        "推荐全州室内景点——工艺展览馆、历史博物馆等。",
        "根据状态灵活调整路线比较好的一天。",
      ],
      [
        "軽い外出を楽しむのに適した天気です。良い一日을! (良い一日を！)",
        "室内中心で計画し、短い散歩程度なら大丈夫な日です。",
        "全州の屋内スポット（工芸品展示館、歴史博物館など）がおすすめ。",
        "コンディションに合わせて柔軟に動線を変えるのが良い日です。",
      ],
    );
  }
}
...
export function PicnicCalendar({ useGeolocation = true }: PicnicCalendarProps) {
  ...
  const forecastTitle = __l(
    "10일 예보",
    "10-Day Forecast",
    "10天预报",
    "10日間予報",
  );
  const rainChanceLabel = __l(
    "강수확률",
    "Rain Chance",
    "降雨概率",
    "降水確率",
  );
  const rainAmountLabel = __l(
    "예상 강수",
    "Expected Rain",
    "预计降雨",
    "予想降水量",
  );
  const outdoorTipLabel = __l(
    "야외 팁",
    "Outdoor Tip",
    "户外提示",
    "屋外アドバイス",
    ...
    <div className="w-full max-w-6xl mx-auto px-4">
        ...
        <div className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-4 sm:gap-6 py-6 relative z-10">
          <AnimatePresence>
            {isLoading
              ...
              : forecast?.daily?.map((dayForecast: ForecastDay, i: number) => {
                  const dayDate = new Date(
                    dayForecast.date.replace(
                      /(\d{4})(\d{2})(\d{2})/,
                      "$1-$2-$3",
                    ),
                  );
                      ...
                      <div className="w-full flex flex-col gap-3 mt-4">
                        ...
                        <div
                          className={cn(
                            ...
                            isRecommended
                              ? "bg-gradient-to-r from-nature-green to-active-blue text-white border-transparent shadow-[0_10px_24px_-12px_rgba(47,111,228,0.45)]"
      ...
      <div className="mt-8 flex justify-end items-center gap-2 px-4 opacity-50">
        ...
        <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-muted-foreground">
          ...
          {language === "ko"
            ? forecast?.metadata?.dataSource || "기상청"
            : language === "zh"
              ? "气象厅"
              : language === "ja"
                ? "気象庁"
                : "KMA"}{" "}
          ({__l("업데이트", "Updated", "更新", "更新")}:{" "}
          {forecast?.metadata?.lastUpdate || "--:--"})
        </span>
      </div>

      {/* Detail Slide Drawer for connecting Live Forecast & History */}
      <PicnicDetailDrawer
        isOpen={drawerOpen}
        onClose={() => setDrawerOpen(false)}
        day={selectedDay}
        locationLabel={translateLocation(forecast?.location)}
        adviceText={selectedDay ? getAdviceText(selectedDay, language) : ""}
      />
