...
Next.js 16 풀스택 웹 애플리케이션입니다. 실시간 협업 코드 에디터, 적응형 간격 반복 학습, AI 웹검색 챗, GPS 기반 날씨 점수화 기능을 하나의 서비스로 통합 제공합니다.
...
| **📚 랩 (Lab)** | 안정도·난이도 기반 적응형 간격 반복 어휘 카드, AI 덱 자동 생성, 웹검색 연동 AI 챗 |
...
| **AI / LLM** | OpenAI 호환 API (범용 + 실험실 이중 구성) |
| **웹 검색** | Tavily API |
...
│       ├── components/          # 39 React 컴포넌트
│       ├── context/             # AuthContext, LanguageContext
│       ├── lib/                 # 25개 모듈, 70+ 비즈니스 로직 파일
│       │   ├── auth/            # 인증 (10개 파일)
│       │   ├── chat/            # AI 챗 (7개 파일)
│       │   ├── lab/             # 적응형 간격 반복 (7개 파일)
...
Nginx 리버스 프록시 설정과 Ubuntu 서버 배포 가이드는 [docs/ubuntu-server-deploy-auth.md](ubuntu-server-deploy-auth.md)를 참고하세요.
...
| 문서 | 설명 |
|------|------|
| [📘 ARCHITECTURE_UML.md](ARCHITECTURE_UML.md) | 아키텍처 및 UML 다이어그램 (English) |
| [📘 ARCHITECTURE_UML_KO.md](ARCHITECTURE_UML_KO.md) | 아키텍처 및 UML 다이어그램 (한국어) |
| [📗 README_KO.md](README_KO.md) | 한국어 README |
| [📗 README_JA.md](README_JA.md) | 日本語のREADME |
| [📗 README_ZH.md](README_ZH.md) | 中文 README |
| [📙 ubuntu-server-deploy-auth.md](ubuntu-server-deploy-auth.md) | Ubuntu 서버 배포 가이드 |
